...
module Aegis
  RSpec.describe TableDataWiper do
    let(:wiper) { described_class.new(s3_cleaner: s3_cleaner) }

    let(:s3_cleaner) { instance_double(S3Cleaner) }

    let(:database) { Database.new('db') }
    let(:table) { Table.new(database, 'table', table_schema, table_location) }
    ...
    describe '#wipe_table_data' do
      ...
      context 'when first partitioning column not given' do
        ...
        it 'raises an error' do
          expect { subject }.to raise_error(PartitionError)
        end
      end
    end
  end
