...
module Aegis
  RSpec.describe Table do
    let(:table) do
      described_class.new(database, table_name, table_schema, table_location,
    ...
    let(:partitions_generator) { instance_double(PartitionsGenerator) }
    let(:table_ddl_generator) { instance_double(TableDDLGenerator) }
    let(:output_downloader) { instance_double(OutputDownloader) }
    let(:table_data_wiper) { instance_double(TableDataWiper) }
    ...
    describe '#download_data' do
      ...
      let(:query_status) { QueryStatus.new('123', :finished, 'query message', output_location) }
      let(:output_location) { QueryOutputLocation.new('s3://bucket/path', 'bucket', 'path') }
