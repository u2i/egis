...
RSpec.describe Egis::TableDataWiper do
  let(:wiper) { described_class.new(s3_cleaner: s3_cleaner) }

  let(:s3_cleaner) { instance_double(Egis::S3Cleaner) }

  let(:database) { Egis::Database.new('db') }
  let(:table) { Egis::Table.new(database, 'table', table_schema, table_location) }
  ...
  describe '#wipe_table_data' do
    ...
    context 'when first partitioning column not given' do
      ...
      it 'raises an error' do
        expect { subject }.to raise_error(Egis::Errors::PartitionError)
      end
    end
  end
end
